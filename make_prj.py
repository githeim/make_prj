...
# @param target_module_name     Target module that is using sub module
...
#
# @return 
def Copy_Module_OutputFiles(Top_Path,target_module_name,sub_module_name,src_config,
        target_config,
        file_list):
    base_src_dir = Top_Path+sub_module_name+'/'+'out'+'/'+src_config
    base_target_dir = Top_Path+target_module_name+'/'+'out'+'/'+target_config
...
# @param module_name[IN] ex) 'SDL2_ttf'
...
def Do_Convert_module_to_absolute_path(types,module_name,module_output_ctx,top_path=Get_Top_Path()):
    prefix = top_path+'/'+module_name  # absolute path's prefix
...
def Get_ext_Module_inst(file_path,file_name):
    ...
    try :
        ...
        print('Err; err on reading module ['+file_name+']'+ ' Cannot find the file ['+file_name+'] on paths ['+str(file_path)+']')
        return (False,None)
    instance=imp.load_module(file_name,fp,pathname,description)
    fp.close()
...
# @param module_name[IN]    module name to know its dependency
...
def Get_all_dependent_list(module_name,top_path=Get_Top_Path(),
        setting_path=DEF_DEFAULT_SETTING_PATH,setting_module=DEF_DEFAULT_SETTING_MODULE):
    module_list = []

    settings_file_path = top_path+'/'+module_name+'/'+setting_path
...
def Get_Module_Output_env_var(module_name,top_path=Get_Top_Path(),
        setting_path=DEF_DEFAULT_SETTING_PATH,setting_module=DEF_DEFAULT_SETTING_MODULE):
    output_env=[]
    settings_file_path = top_path+'/'+module_name+'/'+setting_path
    ...
    try :
        ...
        print('The module ['+module_name+']'+' has no Get_output_common() Function')
    ...
    try :
        ...
        print('The module ['+module_name+']'+' has no Get_output_specific() Function')
    output_env= output_env + Do_Convert_module_to_absolute_path(['lib_path','inc_path'],module_name,Get_output_configs(output_specific,output_common ) )
...
# @param Target_module_name[IN] Target module name to know its dependency
#
# @return 

def Copy_Dependent_Files_on_Sub_module(Target_module_name=Get_Module_Name(),target_config=None):
    ret = Get_all_dependent_list(Target_module_name)
    if (ret[0] == True):
        module_names = ret[1]
    else :
        return (False,None)
    print('module names ='+str(module_names))
    for module_name in module_names:
        connected_config = Get_config_from_dependency_info(module_name,target_config)
        # Get module's output context
        ret =Get_Module_Output_env_var(module_name)
        if (ret[0]==True):
            module_output=ret[1]
        else:
            return (False,None)
        ...
        for output_config in module_output:
            ...
            for item in output_config[1]:
                if item[0]=='bin_file':
                    Copy_Module_OutputFiles(Get_Top_Path(),Target_module_name,module_name,output_config[0],target_config,item[1])
...
#
# @param connected_module_name  ex) SDL2, SDL2_ttf ...the module names
...
#
# @return 
def Get_config_from_dependency_info(connected_module_name,config,dependency_module_info=Get_dependency_module_info()):   
    ret = config
    #find module item and its connected config
    for module in dependency_module_info:
        if (connected_module_name == module[0]):
...
def Do_Setup_Bd_Env(target_config,Target_module_name=Get_Module_Name()):
    ...
    Copy_Dependent_Files_on_Sub_module(Target_module_name,target_config)
    ...
    module_names = Get_dependency_module(Get_dependency_module_info())
    
    for module_name in module_names:
        # Get module's output context
        module_output=[]
        settings_file_path = Get_Top_Path()+module_name+'/'+DEF_DEFAULT_SETTING_PATH 
        ...
        try :
            ...
            print('The module ['+module_name+']'+' has no Get_output_configs() or Get_output_specific() Function')
            continue
        ...
        print('\\n'+'module_name: '+module_name  +' module_output :'+str(module_output)+'\\n')
        for output_config in module_output:
...
def Do_dependent_module_build(target_config,dependency_module_info=Get_dependency_module_info(),top_path=Get_Top_Path()):
    ret = True
    for module_item in dependency_module_info:
        print("Build dependent module ; "+module_item[0] )
        cmd ="cd "+top_path+'/'+module_item[0]+' ; '+'scons CONFIG='+target_config
        output=subprocess.call (cmd, shell=True)
        if (output !=0):
            ret = False
            print("\\033[1;31m Build Error on ["+module_item[0]+"]  \\033[m");
        else:
            print("\\033[1;32m Build Done ; ["+module_item[0]+"]  \\033[m");
    return ret
...
def Get_output_specific():
    return [
            ['x64_Linux_ubuntu',
                [
                    [ 'inc_path',['inc/x64_Linux_ubuntu'] ],
                    [ 'bin_file',['out/x64_Linux_ubuntu/lib/lib"""+getPrjName()+""".so'] ],
                    [ 'lib_path',['out/x64_Linux_ubuntu/lib'] ],
                ]
            ],
            ['x64_Windows', 
                [
                    ...
                    [ 'bin_file',['out/x64_Windows/lib/"""+getPrjName()+""".dll'] ],
...
def makePrjContents(prjName,Prj_Contents):
    ...
    for item in Prj_Contents :
        # make directories
        dir_item = prjName+"/"+item[g_Contents_Offset_Directory]()+"/"
...
def printusage():
    print( "***Usage***")
    print( "$ "+ sys.argv[0]+" project_name homepath_of_the_gtest\n")
    print( "ex) \n")
    print( "$ "+ sys.argv[0]+" myprj /home/myaccount/gtest-3.7.1\n\n")

def main(args):
    print ("C++ Project Template maker")
    print (" by windheim\n\n")
    ...
    makePrjContents(getPrjName(),g_Prj_Contents)
